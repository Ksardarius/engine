...
impl Shader {
    ...
    pub fn get_uniform_location(&self, attr: &str) -> gl::GLint {
        let name: CString = CString::new(attr).unwrap();
        unsafe {
    ...
    pub fn set_int(&self, attr: &str, v0: i32) {
        let attr_loc = self.get_uniform_location(attr);
        unsafe { gl::Uniform1i(attr_loc, v0) };
    }

    pub fn set_float(&self, attr: &str, v0: f32) {
        let attr_loc = self.get_uniform_location(attr);
        unsafe { gl::Uniform1f(attr_loc, v0) };
    }

    pub fn set_vec_3(&self, attr: &str, v0: f32, v1: f32, v2: f32) {
        let attr_loc = self.get_uniform_location(attr);
        unsafe { gl::Uniform3f(attr_loc, v0, v1, v2) };
    }

    pub fn set_vec_3v(&self, attr: &str, vec: &glm::Vec3) {
        let attr_loc = self.get_uniform_location(attr);
